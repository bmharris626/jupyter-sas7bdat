...
- The extension opens/previews `.sas7bdat` files in JupyterLab and supports conversion workflows through the read and convert server endpoints.
...
- Frontend entrypoint is `src/index.tsx`; shared server request helper is `src/request.ts`.
...
- Parquet conversion uses `pyarrow`; SAS labels/formats are stored as Parquet field metadata keys `sas_label` and `sas_format`.
...
- Current route: `POST /jupyter-sas7bdat/convert` with JSON body `{src, dst, format}` where `format` is one of `csv`, `tsv`, `json`, `parquet`, or `xpt`.

## Style And Tooling

- TypeScript is strict (`noImplicitAny`, `noUnusedLocals`, `strictNullChecks`) and emits compiled files to `lib/`.
- Frontend UI uses JupyterLab `ReactWidget`/`DocumentWidget` APIs from `src/index.tsx`; conversion dialog body logic lives in `src/convert.ts` so Jest can test it without loading the full JupyterLab plugin graph.
